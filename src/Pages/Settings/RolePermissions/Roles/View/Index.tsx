...
export default function Index(): JSX.Element {
    ...
    const getRecord = () => {
        ...
        get(`settings/role-permissions/roles/view/${id}`).then((res) => {
            if (res) {
                setRole(res)
            }
        })
        ...
        <div className="mb-3">

            {
                !loading ?
                    ...
                    role ?
                        <div>
                            <AutoTabs title={`${role.name} role`} tabs={tabs} active="permissions" listUrl="/settings/role-permissions/roles" />
                        </div>
                        :
                        <Error404 />
                    :

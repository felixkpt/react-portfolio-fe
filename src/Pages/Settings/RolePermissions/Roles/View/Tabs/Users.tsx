        ...
        <div>
            {
                role && role.id ?
                    <>
                        ...
                        <AutoTable
                            key={key}
                            baseUri={`/settings/users?role_id=${role.id}`}
                            columns={[
                                {
                                    label: 'ID',
                                    key: 'id',
                        ...
                        />
                        <GeneralModal setKey={setKey} title='Add User to Role' actionUrl={`settings/role-permissions/roles/view/${role.id}/add-user`} id={`addUserToRole`}>
...
const AddUser = ({ role }: Pick<Props, 'role'>) => {
    ...
    const loadOptions = async (q: string) => {
        const { data } = await get(`/settings/users?role_id=${role.id}&negate=1&all=1&q=${q}`);

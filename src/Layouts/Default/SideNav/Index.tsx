...
interface CustomEventDetail extends Event {
    detail: string;
}
...
const Index = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            if (expand) {
                expand.removeEventListener('click', collapseEvent);
            }
        };
    }, [currentRole]);
    ...
    const handleSidebarToggle = (e: Event) => toggleSidebar(e as CustomEventDetail);

    useEffect(() => {
        subscribe('hideSideNav', handleSidebarToggle);

        return () => unsubscribe('hideSideNav', handleSidebarToggle);
    }, []);

    useEffect(() => {
        if (window.innerWidth >= 992) {
            toggleSidebar({ detail: 'show' } as CustomEventDetail);
        }
    }, [window.innerWidth]);
...
export const toggleSidebar = (event?: CustomEventDetail, forceClose = false) => {
